...
describe('src/trace', () => {
  ...
  it('logs empty argument', () => {
    ...
    log.trace()
    expect(info.callCount, 'to be', 1)
    expect(info.args[0], 'to equal', ['{"severity":"TRACE","timestamp":"2017-09-01T13:37:42.000Z"}'])
    expect(this.warn.callCount, 'to be', 0)
    expect(this.error.callCount, 'to be', 0)
  })
  ...
  it('logs multiple times', () => {
    ...
    expect(info.args[2], 'to equal', ['{"foo":true,"severity":"TRACE","timestamp":"2017-09-01T13:37:42.000Z"}'])
    expect(this.warn.callCount, 'to be', 0)
    expect(this.error.callCount, 'to be', 0)
  })
  ...
  it('logs single argument via failing promise', async () => {
    ...
    expect(
      info.args[0][0],
      'to match',
      /^\{"message":"wrah","stack":"Error: wrah\\n(.+?)","severity":"TRACE","timestamp":"2017-09-01T13:37:42\.000Z"\}$/
    )
    expect(this.warn.callCount, 'to be', 0)
    expect(this.error.callCount, 'to be', 0)
  })
