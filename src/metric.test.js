...
describe('src/metric.js', () => {
  describe('getMetricRegistry', () => {
    ...
    beforeEach(() => {
      ...
      logMetrics = sinon.stub(MetricRegistry.prototype, 'logMetrics')
    })
    ...
    it('calls logMetrics on an interval', async () => {
      ...
      const times = 4
      clock.tick(60000 * times)
      expect(registry, 'to be an object')
      expect(logMetrics.callCount, 'to be', times)
      expect(logMetrics.args[0], 'to equal', [])
    })
  })
  describe('MetricRegistry', () => {
    ...
    it('creates gauge metric', () => {
      ...
      const labels = { brand: 'vw' }
      let dataPoints = [50, 50, 0, 50]
      dataPoints.map(value => this.metricRegistry.gauge(name, value, labels))
      ...
      dataPoints = [20, 40, 80]
      dataPoints.map(value => this.metricRegistry.gauge(name, value, { brand: 'foo' }))
